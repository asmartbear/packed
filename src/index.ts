...
export class PackedBuffer {
  ...
  writeSerializable(x: Serializable): this {
    ...
    if (this.idx + 10 >= this.length) {    // this "unneeded" check was due to profiler hotspot analysis
  ...
  readSerializable(): Serializable {
    ...
    // if ( t >= TYPE_STRING_LENGTH_ZERO && t <= TYPE_STRING_LENGTH_MAX) {
    //     return decodeString(this.readByteArray(t-TYPE_STRING_LENGTH_ZERO));     // hard-coded length for small encoded string
    // }
    ...
    switch (t) {
      ...
      // case TYPE_OBJECT_REF: { const t = buf.readInteger(); const a = buf.readInteger(); return new ObjectRef(t,a); }
      // case TYPE_OBJECT_REF_NULL: return OBJECT_REF_NULL;
...
/**
 * Reads a Scalar previously written with `writeScalar()`.
 */
/*
export function readScalar(buf: PackedBuffer): Scalar {
  const t = this.buf[buf.idx++];
  // if ( t >= TYPE_STRING_LENGTH_ZERO && t <= TYPE_STRING_LENGTH_MAX) {
  //     return decodeString(this.readByteArray(t-TYPE_STRING_LENGTH_ZERO));     // hard-coded length for small encoded string
  // }
  if (t >= TYPE_SMALLINT_ZERO && t <= TYPE_SMALLINT_MAX) {
  ...
  switch (t) {
    ...
    case TYPE_FALSE: return false;
    case TYPE_TRUE: return true;
    case TYPE_INTEGER: return buf.readInteger();
    case TYPE_SMALLPOS_INTEGER: return buf.readSmallPositiveInteger();
    case TYPE_STRING_LENGTH_ZERO: return "";
    case TYPE_STRING_LENGTH_ONE: return String.fromCharCode(buf.readSmallPositiveInteger());
    case TYPE_STRING: return buf.readString();
    case TYPE_JSON: return buf.readJson() as Scalar;
    // case TYPE_OBJECT_REF: { const t = buf.readInteger(); const a = buf.readInteger(); return new ObjectRef(t,a); }
    // case TYPE_OBJECT_REF_NULL: return OBJECT_REF_NULL;
    case TYPE_UNDEFINED: return undefined;
    case TYPE_POSITIVE_INFINITY: return Number.POSITIVE_INFINITY
